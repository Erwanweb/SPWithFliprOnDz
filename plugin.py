...
# Heartbeat  ---------------------------------------------------
    ...
    def onHeartbeat(self):
        ...
        # Filtration ---------------------------------------------------
        # First we check time needed for filtration
        if self.SPTemp > 32:
            self.PumpTemp = 24
            ...
            # Checking the time of Sun at south
            ...
            # datetime(year, month, day, hour, minute, second, microsecond)
                ...
                Domoticz.Debug("Next middle of period for calculation is fixed at : " + str(self.SunAtSouth))
            ...
            now = datetime.now()
            if self.startpump < now :
                self.pumpon = True
                Domoticz.Debug("Pompe is ON since "+ str(self.startpump))
            else :
                self.pumpon = False
                Domoticz.Debug("Pompe is OFF - Will turn ON at " + str(self.startpump))
            if self.pumpon :
                if self.stoppump < now :
                    self.pumpon = False
