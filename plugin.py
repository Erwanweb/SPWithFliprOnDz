...
class BasePlugin:
    ...
    def onStart(self):
        ...
        devicecreated = []
        if 1 not in Devices:
            Options = {"LevelActions": "||", "LevelNames": "Off|Auto|Forced", "LevelOffHidden": "false",
                       "SelectorStyle": "0"}
            Domoticz.Device(Name="Filtration control", Unit=1, TypeName="Selector Switch", Switchtype=18, Image=15,
                            Options=Options, Used=1).Create()
            devicecreated.append(deviceparam(1, 0, "0"))  # default is Off state
        if 2 not in Devices:
            Options = {"LevelActions": "||", "LevelNames": "Off|Auto|Forced", "LevelOffHidden": "false",
                       "SelectorStyle": "0"}
            Domoticz.Device(Name="Heating control", Unit=2, TypeName="Selector Switch", Switchtype=18, Image=15,
                            Options=Options, Used=1).Create()
        ...
        if 12 not in Devices:
            Options = {"LevelActions": "||", "LevelNames": "Off|Auto|Manual", "LevelOffHidden": "false",
                       "SelectorStyle": "0"}
            Domoticz.Device(Name="Water treatment control", Unit=12, TypeName="Selector Switch", Switchtype=18,
                            Image=15, Options=Options, Used=1).Create()
    ...
    def verifSP(self):
        AlexaAPI(
            "La piscine est a {} degré, son P-H est {} avec une valeur de {} et le niveau de desinfectant est {} avec une valeur redox de {}".format(
                self.SPTemp, self.PHText, self.PHValNet, self.RedoxText, self.RedoxVal))
