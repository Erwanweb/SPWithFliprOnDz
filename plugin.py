...
# Heartbeat  ---------------------------------------------------
    ...
    def onHeartbeat(self):
        ...
        if self.nextRefresh <= now:
            ...
            self.SPTemp = round(str(self.tempVal))  # pas de virgule
            self.PHValNet = round(str(self.PHVal),1)  # 1 chiffre apres la virgule
            # Updating devices values
            Domoticz.Debug("Updating Devices from Flipr Values")
            Devices[4].Update(nValue = 0,sValue = str(self.tempVal))
