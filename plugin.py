...
# Heartbeat  ---------------------------------------------------
    ...
    def onHeartbeat(self):
        ...
        if Devices[1].sValue == "0": # Off
            ...
            # If Auto : First we check time needed for filtration
            if self.SPTemp > 31:
                self.PumpTemp = 24
                ...
                # datetime(year, month, day, hour, minute, second, microsecond)
                    ...
                    self.startpump = self.SunAtSouth - timedelta(hours=(self.PumpTemp / 2))
                    Domoticz.Debug("Pump start with default value at : " + str(self.startpump))
                    self.stoppump = self.startpump + timedelta(hours=(self.PumpTemp))
                    Domoticz.Debug("Pump stop with default value at : " + str(self.stoppump))
                # And now, we check state and turn on or off the pump
                now = datetime.now()
                if self.startpump < now and self.stoppump > now :
                    self.pumpon = True
                    if Devices[3].nValue == 0:
                        Devices[3].Update(nValue=1, sValue=Devices[3].sValue)
        ...
        # Temp ---------------------------------------------------
        # if Devices[1].sValue =! "0":  # On in manual or in Auto
        # self.setpoint = float(Devices[5].sValue)
        #   if SPTemp < (float(Devices[5].sValue) - 0.5):
        #       self.Heating = True
        #   else :
        #       if self.Heating and SPTemp => float(Devices[5].sValue):
        #           self.Heating = False
        # else :
