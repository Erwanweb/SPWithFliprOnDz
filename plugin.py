...
# Heartbeat  ---------------------------------------------------
    ...
    def onHeartbeat(self):
        ...
        # Updating Flipr mesured values (the device update it to the server about each 15 minutes)
        if self.nextRefresh <= now:
            self.nextRefresh = now + timedelta(minutes=5)
            self.handleConnection()
            ...
            # On arrondit les valeurs de temp sans virgules
        ...
        # datetime(year, month, day, hour, minute, second, microsecond)
           #now = datetime.now()
           SunAtSouthHour = int(jsonData['SunAtSouth'][0])
           SunAtSouthMin = int(jsonData['SunAtSouth'][1])
           self.SunAtSouth = datetime.now()
           #self.SunAtSouth = time(hour=int(jsonData['SunAtSouth'].split(':')[0]), minute=int(jsonData['SunAtSouth'].split(':')[1]), second = 0)
           #self.SunAtSouth = datetime(year=int(jsonData['ServerTime'].split('-')[0]), month=int(jsonData['ServerTime'].split('-')[1]), day=int(jsonData['ServerTime'].split('-')[2]), hour=int(jsonData['SunAtSouth'].split(':')[0]), minute=int(jsonData['SunAtSouth'].split(':')[1]), second = 0)
           #self.SunAtSouth = datetime(year=now.strftime("%Y"), month=now.strftime("%m"), day=now.strftime("%d"), hour=13, minute=0, second=0)
           #self.SunAtSouth = datetime(year=datetime.now().strftime("%Y"), month=datetime.now().strftime("%m"), day=datetime.now().strftime("%d"), hour=13, minute=0, second=0)
        #self.SunAtSouth = time(hour = 13, minute = 58, second = 0)
        Domoticz.Debug("Sun at south is at  : {} H {}" format(SunAtSouthHour,SunAtSouthMin))
        # self.nextcalc = now + timedelta(minutes=self.calculate_period)
        self.startpump = self.SunAtSouth - timedelta(hours=(self.PumpTemp/ 2))
        Domoticz.Debug("Pump start at : " + str(self.startpump))
        self.stoppump = self.startpump + timedelta(hours=(self.PumpTemp))
        Domoticz.Debug("Pump stop at : " + str(self.stoppump))
