...
class BasePlugin:
    ...
    def onStart(self):
        ...
        if 9 not in Devices:
            ...
            devicecreated.append(deviceparam(9, 0, ""))  # default is clear
...
# Heartbeat  ---------------------------------------------------
    ...
    def onHeartbeat(self):
        ...
        if self.nextRefresh <= now:
            ...
            self.SPTemp = round(self.tempVal)  # pas de virgule
            self.PHValNet = round(self.PHVal,1)  # 1 chiffre apres la virgule
            # Updating devices values
            Domoticz.Debug("Updating Devices from Flipr Values")
            Devices[4].Update(nValue = self.SPTemp,sValue = str(self.SPTemp))
            Devices[7].Update(nValue= 0, sValue=str(self.PHText))
            Devices[8].Update(nValue= self.PHValNet, sValue=str(self.PHValNet ))
            Devices[9].Update(nValue= 0, sValue=str(self.RedoxText))
            Devices[10].Update(nValue= self.RedoxVal, sValue=str(self.RedoxVal))
            Devices[11].Update(nValue= 0, sValue=str(self.batVal))

        # Regul regul
        # Filtration :
        self.PumpTemp = (self.SPTemp / 2)
        # Temp
